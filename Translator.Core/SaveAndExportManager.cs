    ...
    public static class SaveAndExportManager
    {
        ...
        private static void CompareAndAggregateTranslationAndTemplate(FileData lines, FileData templates, ref FileData results)
        {
            foreach (LineData lineData in templates.Values)
            {
                ...
                {
                    if (translatedLineData.Translation == lineData.Template)
                    {
                        results.Add(lineData.EekID, lineData);
                    }
                    else if (Settings.Default.ExportTranslatedWithMissingLines)
                    {
                        var tempLine = new LineData(translatedLineData, new EekStringID(translatedLineData.ID, translatedLineData.Category));
                        tempLine.Translation = "@@@TN: " + tempLine.Translation;
        ...
        public static void SortIntoCategories(ref List<CategorizedLines> CategorizedStrings, FileData IdsToExport, FileData translationData)
        {
            foreach (LineData item in IdsToExport.Values)
            {
                ...
                if (translationData.TryGetValue(item.EekID, out LineData? TempResult))
                {
                    ...
                    {
                        item.Translation = TempResult.TranslationLength > 0 ? TempResult.Translation : item.Template.RemoveVAHints();
                    }
                }
                else
                {
                    item.Translation = item.Template.RemoveVAHints();
                }
