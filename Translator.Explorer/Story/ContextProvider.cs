...
public sealed class ContextProvider
{
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (nodes[i].Type == NodeType.Criterion && nodes[i].Data is not null)
                {
                    ...
                    switch (criterion.CompareType)
                    {
                        ...
                        {
                            result = nodes.Find((n) => n.Type == NodeType.CriteriaGroup && n.ID == criterion.Value);
                            if (result is not null)
                            {
                                nodes[i].AddParentNode(result);
                                break;
                        ...
                        {
                            result = nodes.Find((n) => n.Type == NodeType.Dialogue && n.FileName == criterion.Character && n.ID == criterion.Value);
                            if (result is not null)
                            {
                        ...
                        {
                            result = nodes.Find((n) => n.Type == NodeType.ItemGroup && n.Text == criterion.Key);
                            if (result is not null)
                            {
                                nodes[i].AddParentNode(result);
                                break;
                        ...
                        {
                            result = nodes.Find((n) => n.Type == NodeType.Personality && n.FileName == criterion.Character && n.ID == ((PersonalityTraits)int.Parse(criterion.Key!)).ToString());
                            if (result is not null)
                            {
                                nodes[i].AddParentNode(result);
                                break;
                        ...
                        {
                            result = nodes.Find((n) => n.Type == NodeType.Quest && n.ID == criterion.Key);
                            if (result is not null)
                            {
                                nodes[i].AddParentNode(result);
                                break;
                ...
                else if (nodes[i].Type == NodeType.Event && nodes[i].Data is not null)
                {
                    ...
                    switch (gameEvent.EventType)
                    {
                        ...
                        {
                            result = nodes.Find((n) => n.Type == NodeType.Event && n.Text == gameEvent.Value);
                            if (result is not null)
                            {
                ...
                else if (nodes[i].Type == NodeType.EventTrigger && nodes[i].Data is not null)
                {
                    ...
                    foreach (GameEvent _event in trigger.Events!)
                    {
                        ...
                        else
                        {
                            ...
                            nodes.Add(eventNode);
                            nodes[i].AddChildNode(eventNode);
                        }
                    }
    ...
    private void MergeDoors(NodeList nodes)
    {
        ...
        {
            result = nodes.Find((n) => n.ID == door.ID);
            if (result is not null)
            {
